...
const activeSessions = ref([
  { id: 101, patient: patients.value[0], equipement: "Pédalier (Bras)", scenario: "L'Aube Douce", rpm: 45, bpm: 112, timeElapsed: "12:45", timeTotal: "20:00", isStopped: false }
])
...
const generateCalendar = () => {
  ...
  for (let i = 1; i <= lastDayOfMonth.getDate(); i++) {
    ...
    let dayEvents = []
    if (i % 3 === 0 && i < 28) {
      dayEvents.push({ patient: patients.value[0], time: "10:00", color: "#00B8D9", scenario: "L'Aube Douce", duration: "20 min" })
    }
    if (i % 5 === 0) {
